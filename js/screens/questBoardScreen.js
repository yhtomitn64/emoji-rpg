...
function render() {
  const rows = Object.keys(QUEST_REQUIREMENTS).map((monsterId) => {
    ...
    return `<div class="quest-row">
      <span data-tooltip="${describeItem(state, rewardItemId)}">${monster.emoji} ${monster.name} Lv.${level}: ${progress}/${required} killed — reward: ${rewardItem.emoji} ${rewardItem.name} ×${rewardQuantity}</span>
